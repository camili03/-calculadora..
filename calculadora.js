...
const botonDecBin = document.querySelector('[data-DecBin]')
const botonDecOct = document.querySelector('[data-DecOct]')
const botonDecHex = document.querySelector('[data-DecHex]')
const botonRaiz = document.querySelector('[data-operador')
...
class Calculadora {
    ...
    realizarCalculo() {
        ...
        switch (this.operador) {
            ...
            break
            case'√':
            resultado= Math.sqrt(conversionValorInferior)
            break
            default: return
        }
...
function convertToBinary (conversionValorInferior) {
    if (conversionValorInferior > 0) {
        return convertToBinary( parseInt(conversionValorInferior / 2) ) + (conversionValorInferior % 2)
    };
    return '';
}
...
botonDecBin.addEventListener('click' ,() =>{
    convertToBinary ()
})
...
botonRaiz.addEventListener('click',() => {
    botonRaiz = Math.sqrt(conversionValorInferior)
})
